...
async function handleCreateRace() {
	...
	console.log(`player: ${player_id} track: ${track_id}`);
	...
	try {
        ...
        const { Track: track, Cars: racers } = result;
        renderAt("#race", renderRaceStartView(track, racers));
		let { ID: race_id } = result;
		// need to subtract 1 to fix issue with API as https://github.com/udacity/nd032-c3-asynchronous-programming-with-javascript-project-starter/issues/6
        store.race_id = race_id - 1;
    } catch (e) {
    ...
    try {
        ...
        console.error("Error starting race", e.message);
    }

    const results = await runRace(store.race_id);
    renderAt("#race", resultsView(results));
...
async function runCountdown() {
	try {
		...
		console.log('Error on CountDown',error);
	}
}
